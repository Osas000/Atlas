...
class TestAtlasKernel:
    ...
    async def test_boot(self, kernel: AtlasKernel) -> None:
        ...
        assert kernel.registry.count == 14  # memory_manager + operations_core + opportunity_engine + mission_control + notification_service + agent_runtime + multi_agent_runtime + persistence_manager + system_monitor + monitoring_api + plugin_manager + connector_manager + workflow_engine + security_manager
    ...
    async def test_health_check(self, kernel: AtlasKernel) -> None:
        ...
        assert health.healthy_services == 15  # healthy + memory_manager + operations_core + opportunity_engine + mission_control + notification_service + agent_runtime + multi_agent_runtime + persistence_manager + system_monitor + monitoring_api + plugin_manager + connector_manager + workflow_engine + security_manager
